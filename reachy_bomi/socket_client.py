...
Usage:
    python3 bomi_client.py <robot_ip> [--port 5051] [--cam 0]
...
def check_region_cursor(crs_x: float, crs_y: float) -> int:
    """
    Returns region 1–9 based on 3×3 grid over BASE_WIDTH×BASE_HEIGHT.
    Layout:
        1 | 2 | 3   (top row)
        4 | 5 | 6   (middle row)
        7 | 8 | 9   (bottom row)
...
class BoMIMap:
    ...
    def transform(self, features: np.ndarray) -> tuple:
        body = self.scaler.transform([features])[0]
        cu = np.dot(body, self.pca.components_.T)
